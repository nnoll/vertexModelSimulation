function [ rV, d0, c2v, bndryVs, bndryEs, bndryCs, T, T_ext, rB_ext, Tri, q ] = rectLattice( N, L, alpha )
    ...
    minY = min(q0(:,2)); maxY = max(q0(:,2)); midY = .5*(minY+maxY);
    ...
    delta = (maxY - minY)/5;
    bndry = [midX,maxY;maxX,midY+delta;maxX,midY-delta;midX,minY;minX,midY-delta;minX,midY+delta];
...
%     scatter(rV(:,1),rV(:,2),'bo')
%     hold all
    dV = zeros(size(rV,1),size(bndry,1));
    for ii = 1:size(bndry,1)
        p = mod(ii,size(bndry,1)) + 1;
        pos = .5*(bndry(ii,:) + bndry(p,:));
        vec1 = bndry(p,:) - bndry(ii,:);
        plane = [pos,0,vec1,0,0,0,1];
        for jj = 1:size(rV,1)
           dV(jj,ii) = generate.distancePointPlane([rV(jj,:),0],plane);
        end
    end
    bndryVs = min(abs(dV),[],2) < 1e-4;
%     scatter(rV(bndryVs,1),rV(bndryVs,2),'r','filled')
%     pause
    rV_ext = rV(bndryVs,:);
