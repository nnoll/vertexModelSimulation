...
% RETURN STRUCT 
    ...
    for b = 1:size(d0,1)
        ...
        Struct.Bdat(b).radius = T(b)/abs(dP(b));
        
        if (Struct.Bdat(b).curvature > 0) % Compute rBar
            deltaVec = d1(:,b)' * rV;
            posB = .5*abs(d1(:,b)') * rV;
            Lb = sqrt(sum(deltaVec.^2));
            deltaVec = deltaVec / Lb;
            deltaVec = sign(dP(b)) * deltaVec;
%             Struct.Bdat(b).radius
%             (Lb/2)
%             pause
            deltaVec = abs(sqrt((Struct.Bdat(b).radius/10)^2 - (Lb/2)^2)) * deltaVec;
            Struct.Bdat(b).rBar = posB + deltaVec;
        else
